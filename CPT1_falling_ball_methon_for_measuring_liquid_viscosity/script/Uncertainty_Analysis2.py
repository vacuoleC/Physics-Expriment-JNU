...
class ExperimentConfig:
    ...
    """
    # ---------------------- 基础物理参数（实验测量/给定值）----------------------
    rhop = 0.956          # 油的密度 (g/cm³) → 自动转 kg/m³
    rho = 7.76            # 小球密度 (g/cm³) → 自动转 kg/m³
    L = 15.0              # 小球匀速下落距离 (cm) → 自动转 m
    H = 34.1              # 液柱高度 (cm) → 自动转 m
    D = 2.23              # 容器内径 (cm) → 自动转 m
    d0 = 0.050            # 千分尺零点误差 (cm) → 自动转 m
    T1 = 23.7             # 温度测量值1 (℃)（仅记录，不参与计算）
    T2 = 23.8             # 温度测量值2 (℃)（仅记录，不参与计算）
    ...
    measure_groups = {
        "小球组": {
            "d_cm": [0.1952, 0.1947, 0.1910, 0.1910, 0.1948],
            "t_s": [9.03, 8.91, 8.87, 8.81, 8.75]
        },
        "大球组": {
            "d_cm": [0.2941, 0.2943, 0.2952, 0.2952, 0.2948],
            "t_s": [4.15, 4.19, 4.28, 4.21, 4.22]
        }
    }
